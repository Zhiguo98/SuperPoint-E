...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--input_pairs', type=str, default='assets/wildtrack.txt',
        help='Path to the list of image pairs')
    parser.add_argument(
        '--input_dir', type=str, default='assets/wildtrack1_images/',
    ...
    parser.add_argument(
        '--resize', type=int, nargs='+', default=[1920, 1080],
    ...
    parser.add_argument(
        '--matchtype', choices={'superglue', 'bruteforce', 'ransac', 'disk', 'colmap'}, default='superglue',
    ...
    parser.add_argument(
        ...
        help='SuperGlue modified weights')
    parser.add_argument(
    ...
    # pairs_matched = [[x[:15],x[16:-12]] for x in os.listdir(opt.output_dir)]
    ...
    for i, pair in enumerate(pairs):
        ...
        image0, inp0, scales0 = read_image_modified(
            input_dir / name0, device, opt.resize, rot0, opt.resize_float)
        image1, inp1, scales1 = read_image_modified(
            input_dir / name1, device, opt.resize, rot1, opt.resize_float)
        ...
        # import cv2
        # print(inp0.detach().cpu().numpy().shape)
        # cv2.imwrite("/Users/zhiguoma/Desktop/master/homework/image_understanding/result/output/img_from_colmap.png",inp0.detach().cpu().numpy()[0,0,:,205:-155]*255.)
        if do_match:
