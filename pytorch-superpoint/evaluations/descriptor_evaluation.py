...
import sys
sys.path.append('/Users/zhiguoma/Desktop/master/homework/image_understanding/final/SuperPointTrackingAdaptation/pytorch-superpoint')
...
def compute_homography(data, keep_k_points=1000, correctness_thresh=3, orb=False, shape=(240,320)):
    ...
    #                                   warped_keypoints[:, 1]]
    desc = data['desc'].astype('float32')
    warped_desc = data['warped_desc'].astype('float32')
    ...
    if desc.shape[0]>0 and warped_desc.shape[0]>0:
        cv2_matches = bf.match(desc, warped_desc)
        #print(desc.dtype,desc.shape,warped_desc.dtype,warped_desc.shape,len(cv2_matches))
        #exit(1)
        ...
        cv2_matches = np.array([])
        m_dist = np.array([1000])
        matches = np.array([])
        H = None
        inliers = np.array([])
        mean_dist = 1000
    ...
    # Compute correctness
    if H is None:
        correctness = np.array([False,False,False,False,False,False])
        H = np.identity(3)
        mean_dist = 1000
        ...
        #print(H,real_H,correctness_thresh,mean_dist,correctness.shape)
        #exit(1)
