...
class SuperPointNet(torch.nn.Module):
  ...
  def forward_features(self, x, subpixel=False):
    """ Forward pass that jointly computes unprocessed point and descriptor
    tensors.
    Input
      x: Image pytorch tensor shaped N x 1 x H x W.
    ...
    if self.reBn:
        # Shared Encoder.
        features_conv1a = self.conv1a(x)
        features_bn1a = self.bn1a(features_conv1a)
        features_relu = self.relu(features_bn1a)
        conv1 = self.relu(self.bn1b(self.conv1b(features_relu)))
        x, ind1 = self.pool(conv1)
        x = self.relu(self.bn2a(self.conv2a(x)))
        conv2 = self.relu(self.bn2b(self.conv2b(x)))
        x, ind2 = self.pool(conv2)
    ...
    dn = torch.norm(desc, p=2, dim=1) # Compute the norm.
    desc = desc.div(torch.unsqueeze(dn, 1)) # Divide by norm to normalize.
    output = {'semi': semi, 'desc': desc, 'conv': features_conv1a, 'bn': features_bn1a, 'relu': features_relu}
...
if __name__ == '__main__':
    ...
    #     "/Users/zhiguoma/Desktop/master/homework/image_understanding/final/SuperPointTrackingAdaptation/pytorch-superpoint/pretrained/superpoint_v1.pth"))
    model.load_state_dict(torch.load(
    ...
    color_image_path = "/Users/zhiguoma/Desktop/master/homework/image_understanding/dataset/EndoJPEG/hyperK_000/00012.jpg"
